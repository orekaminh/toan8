...
(function () {
    ...
    blocks.forEach(function (block) {
      ...
      if (hintBtn) {
        hintBtn.addEventListener("click", function () {
          ...
          var b = MATH9.el("div", { class: "hint-bubble" });
          b.innerHTML =
  ...
  function bindQuiz(root, lesson) {
    ...
    root.querySelectorAll(".quiz-q").forEach(function (block) {
      ...
      block.querySelector('[data-act="check"]').addEventListener("click", function () {
        ...
        if (MATH9.store) {
          MATH9.store.recordQuiz(lesson.id, idx, correct);
          var mid = lesson.id + ":q" + idx;
          if (correct) MATH9.store.removeMistake(mid);
          else MATH9.store.addMistake({ id: mid, lessonId: lesson.id, lessonTitle: lesson.title, q: q.q,
  ...
  function bindDone(lesson) {
    ...
    if (!btn || !MATH9.store) return;
    function refresh() {
      var d = MATH9.store.isDone(lesson.id);
      btn.textContent = d ? "✓ Đã học bài này" : "Đánh dấu đã học bài này";
      btn.className = "btn-done" + (d ? " is-done" : "");
    }
    btn.addEventListener("click", function () { MATH9.store.setDone(lesson.id, !MATH9.store.isDone(lesson.id)); refresh(); });
    refresh();
  }

  // ---- Render chính -------------------------------------------------------
  MATH9.renderLessonPage = function () {
    var id = getParam("id");
    var lesson = id ? MATH9.getLesson(id) : null;
    ...
    if (lesson.visual && lesson.visual.type && MATH9.Widgets) {
      var mount = document.getElementById("widget-mount");
      var fn = MATH9.Widgets[lesson.visual.type];
    ...
    bindDone(lesson);
    if (MATH9.store) MATH9.store.markVisited(lesson.id);
  };
})();
