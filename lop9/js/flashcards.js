...
(function () {
  ...
  function buildQueue() {
    ...
    var due = pool.filter(function (c) { return (MATH9.store ? MATH9.store.getCard(c.id).due : 0) <= now; });
  ...
  function grade(ok) {
    if (MATH9.store) MATH9.store.gradeCard(queue[pos].id, ok);
    pos++; flipped = false; render();
  }

  MATH9.renderFlashcards = function () {
    collect();
